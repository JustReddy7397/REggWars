...
public class KillMessageManager {
    ...
    public void register(File file) {
        for (Class<? extends KillMessage> messages :
            ...
            if (this.messages.containsKey(message.getId())) continue;
            this.messages.put(message.getId(), message);
            ChatUtil.sendConsole("&7[&dReggWars&7] &aRegistered the &l" + message.getSubname() + " " +
                    "(" + message.getId() + ") &akill message");
        }
    }
    ...
    public KillMessage getByPermission(String permission) {
        for (KillMessage message : messages.values()) {
            if (message.getPermission().equalsIgnoreCase(permission)) return message;
        }
        return null;
    }
}
